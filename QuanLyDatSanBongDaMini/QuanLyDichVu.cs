    ...
    public partial class QuanLyDichVu : Form
    {
            ...
            try
            {
                ...
                if (result != null)
                {
                    ...
                    if (soLuongTonKho >= soLuongThem)
                    {
                        ...
                        command.CommandText = "INSERT INTO DichVu (TenDichVu, Soluong, DonGia, Tongtien, SoLuongTonKho) VALUES (@TenDichVu, @Soluong, @DonGia, @Tongtien, @SoLuongTonKhoMoi)";
                        command.Parameters.Clear();
                        command.Parameters.AddWithValue("@TenDichVu", tenDichVu);
                        command.Parameters.AddWithValue("@Soluong", soLuongThem);
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int oldQuantity = Convert.ToInt32(dataGridView1.CurrentRow.Cells["Soluong"].Value);
                ...
                if (newStockQuantity >= 0)
                {
                    ...
                    command = connection.CreateCommand();
                    command.CommandText = "UPDATE DichVu SET Soluong=@Soluong, SoLuongTonKho=@SoLuongTonKho WHERE MaDV=@MaDV";
                    command.Parameters.AddWithValue("@MaDV", maDV);
                    command.Parameters.AddWithValue("@Soluong", newQuantity);
